...
import com.example.shareappsettingswithgiraffe.ui.components.CreatePathForGradient
...
@Composable
fun DrawGraphItself(
    graphColor: Color = Color.Green, dataList: List<Float>, listOfDatesRaw: List<String> = listOf(
        "13.05.2022", "13.05.2027", "13.05.2030", "13.06.2004", "13.06.2022"
    ), paddingForGraph: PaddingValues = PaddingValues(start = 60.dp, end = 40.dp, bottom = 20.dp)
) {


    val dataListCorrected = if (dataList == emptyList<Float>()) {
...
//    println("DrawGraphItself:  offSetDateTime is $offSetDateTime")
    ...
    val listOfDates = listOfDatesRaw
...
//            .padding(start = 60.dp, end = 40.dp, bottom = 20.dp)
            ...
            .drawWithCache {
                ...
                // Inside GeneratePathNew() fun the data is also transformed( the data occupies the whole area and also rotated)
                // with TransformTheData() fun
                val path = GeneratePathNew(dataListCorrected, size)
                ...
                onDrawBehind {
                    ...
                    repeat(amountOfDates) { i ->
                        ...
                        val measuredText = textMeasurer.measure(
                            AnnotatedString(listOfDates[i]), style = TextStyle(
                                fontSize = 10.sp, color = Color(0xFF929292)
                            )
                        )
                        drawText(
                    ...
                    val minValue = dataListCorrected.min()
                    val maxValue = dataListCorrected.max()
                    val differenceInValues = maxValue - minValue
                    val verticalStepForValues: Float = differenceInValues / (horizontalLines + 1)
                    ...
                    repeat(listOfValuesToDraw.size) { j ->
                        ...
                        val roundedValue =
                            ((listOfValuesToDraw[j].toDouble() * 100.0).roundToInt() / 100.0).toString()
                        val measuredText = textMeasurer.measure(
                            AnnotatedString(
//                                    listOfValuesToDraw[j].toString()
                                roundedValue
                            ), style = TextStyle(
                                fontSize = 10.sp, color = Color(0xFF929292)
                            )
                        ...
                        drawText(
                            textLayoutResult = measuredText,
                            topLeft = Offset(
                                x = 0f - (measuredText.size.width * 1.3f), y = startY - 20
                            ),
                        )
                    }
                    ...
                    drawPath(
                        path = path, color = graphColor, style = Stroke(2.dp.toPx())
                    )

                    //Drawing the gradient
                    val brush = Brush.verticalGradient(
                        listOf(
                            graphColor.copy(alpha = 0.4f), Color.Transparent
                        )
                    )
                    drawPath(
                        path = filledPath, brush = brush, style = Fill
                    )
                }
            })
    }
...
fun GeneratePathNew(data: List<Float>, size: Size): Path {
    val path = Path()

    // transform the data so that it rotated and scaled
    val transformedData = TransformTheDataNew(data, size)
...
fun TransformTheDataNew(data: List<Float>, size: Size): List<Float> {
    ...
    if (maxValue <= heightOfDrawingArea) {
        ...
        scaledData1 = alignedData.map { value ->
            value * coefficientY
        }
    } else {
...
fun CreatePathForGradientNew(path: Path, size: Size): Path {
